 ...
 * @param type $end_hour
 * @param type $minute_ranges
 * @param type $default_hour
 * @param type $default_minute
 * @return string
 */
function magia_time_select_generator($start_hour = 0, $end_hour = 23, $minute_ranges = 30, $default_time = '08:00:00') {

    $start_hour = (isset($start_hour) && $start_hour != '' ) ? $start_hour : 9;
    $end_hour = (isset($end_hour) && $end_hour != '' ) ? $end_hour : 23;
    $minute_ranges = (isset($minute_ranges) && $minute_ranges != '' ) ? $minute_ranges : 30;

    // Desglosar el tiempo predeterminado
    list($default_hour, $default_minute, $default_second) = explode(':', $default_time);
    ...
    for ($h = $start_hour; $h <= $end_hour; $h++) {
        ...
        for ($m = 0; $m <= 59; $m += $minute_ranges) {

            $minute = str_pad($m, 2, '0', STR_PAD_LEFT);
            $hour = str_pad($h, 2, '0', STR_PAD_LEFT);

            // Establecer como seleccionado el tiempo por defecto si coincide
            $selected = ($h == $default_hour && $m == $default_minute) ? " selected " : "";

            $html .= '<option value="' . $hour . ':' . $minute . ':00"' . $selected . '>' . $hour . ' : ' . $minute . '</option>';
        }
    }
