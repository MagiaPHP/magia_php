 ...
 * Date: 2020-04-22
 */
function viewr($c, $a, $arg = array("title" => "Magia_php")) {
    global $config_public_theme;

    // Si no hay configuración, usamos la por defecto     
    $public_theme = _options_option('config_public_html_theme') ?? 'Rapid';
    ...
    switch ($c) {
        case "public_html":
            if (file_exists(WWW_E . "/public_html/$public_theme/views/$a.php")) {
                include WWW_E . "/public_html/$public_theme/views/$a.php"; // Incluir la vista
            } else {
                include WWW . "/public_html/$public_theme/views/$a.php"; // Incluir la vista de la ruta alternativa
            }
            break;

        default:
            if (file_exists(WWW_E . "/$c/views/$a.php")) {
                include WWW_E . "/$c/views/$a.php"; // Incluir la vista
            } else {
                include WWW . "/$c/views/$a.php"; // Incluir la vista de la ruta alternativa
            }
            break;
    }
    ...
    return $output; // Retornar el contenido de la vista
}
