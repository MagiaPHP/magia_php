...
$date_creation = (isset($_POST["date_creation"]) && $_POST["date_creation"] !="" ) ? clean($_POST["date_creation"]) : current_timestamp() ;
...
if (!$date_creation) {
    array_push($error, 'Date creation is mandatory');
}
if (!$order_by) {
...
if (! hr_documents_is_date_creation($date_creation) ) {
    array_push($error, 'Date creation format error');
}
...
if (! $error ) {
    ...
    hr_documents_edit(
        $id ,  $code ,  $title ,  $content ,  $version ,  $date_creation ,  $order_by ,  $status    
        );
...
switch ($redi["redi"]) {
        ...
        case 5: // custom
            // ejemplo index.php?c=hr_documents&a=ok_edit&redi[redi]=5&redi[c]=contacts&redi[a]=details&redi[params]=id%3D1022
