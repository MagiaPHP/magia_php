
<form method="post" action="index.php">
    <input type="hidden" name="c" value="_menus">
    <input type="hidden" name="a" value="ok_show_col_from_table">
    <input type="hidden" name="redi" value="1">

    <table class="table table-bordered">
        <tr>
            <?php
            //$checked_array = json_decode(_options_option("config__menus_show_col_from_table"), true);

            // Obtiene la opción de configuración como una cadena
            $config_menus_show_col_json = _options_option("config__menus_show_col_from_table");

            // Verifica si la cadena es un JSON válido y decodifica. Si no es válido, asigna un array vacío
            $checked_array = is_json($config_menus_show_col_json) ? json_decode($config_menus_show_col_json, true) : [];

            foreach (_menus_db_col_list_from_table($c) as $th) {
                // si hay como parte del array $checked_array
                // o si el array tiene cero elementos (au no registrado)
                $checked = (isset($checked_array[$th]) || !isset($checked_array) ) ? " checked " : "";
                echo '<td><input ' . $checked . ' type="checkbox" name="' . $th . '" id="' . $th . '"> ' . $th . ' </td>';
            }
            ?>
            <td>
                <button type="submit" class="btn btn-default"><?php _t("Save"); ?></button>
            </td>
        </tr>
    </table>
</form>




