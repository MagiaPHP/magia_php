...
<table class="table table-striped table-condensed table-bordered">
    ...
    <thead>
        <tr class="info">
            ...
            $user_cols_array = order_by_get_user_or_default_columns("veh_vehicles_drivers");

            //vardump($user_cols_array); 
            // Renderizar encabezados con iconos de ordenación
            order_by_render_table_headers_with_icons($user_cols_array, "veh_vehicles_drivers", $order_col, $order_way);
            ?>
        </tr>
    </thead>
    ...
    <tbody>
        ...
        foreach ($veh_vehicles_drivers as $veh_vehicles_drivers_item) {
            ...
            $veh_vehicles_drivers_line = new Veh_vehicles_drivers();
            $veh_vehicles_drivers_line->setVeh_vehicles_drivers($veh_vehicles_drivers_item["id"]);
                

            // Verificar si tiene comentarios
            $has_comments = (comments_total_by_controller_id("veh_vehicles_drivers", $veh_vehicles_drivers_line->getId())) ? true : false;
            ...
            if (method_exists($veh_vehicles_drivers_line, "getDate")) {
                // Llamar al método getDate() si existe
                $date = $veh_vehicles_drivers_line->getDate();
            ...
            foreach ($user_cols_array as $col) {
                ...
                if ($col["show"]) {
                    ...
                    switch ($field) {
                        ...
                        break;
                     case 'vehicle_id':
                        echo '<td><a href="index.php?c=veh_vehicles&a=details&id=' . $veh_vehicles_drivers_item['vehicle_id'] . '">' . veh_vehicles_field_id("name", $veh_vehicles_drivers_item['vehicle_id'] ?? null) . '</a></td>';
                        break;
                     case 'driver_id':
                            // Obtener id de oficina, si no existe se asigna null
                            $driver_id = $veh_vehicles_drivers_line->getDriver_id() ?? null;
                            ...
                            break;
                     case 'date_start':
                        echo '<td>' . magia_dates_formated($veh_vehicles_drivers_item[$field]) . '</td>';
                        break;
                     case 'date_end':
                        echo '<td>' . magia_dates_formated($veh_vehicles_drivers_item[$field]) . '</td>';
                        break;
                     case 'notes':
                        echo '<td>' . ($veh_vehicles_drivers_item[$field]) . '</td>';
                        break;
                     case 'date_registre':
                        echo '<td>' . magia_dates_formated($veh_vehicles_drivers_item[$field]) . '</td>';
                        break;
                    ...
                    case "total_plus_vat":
                        echo '<td  class="text-right">' . monedaf($veh_vehicles_drivers_item["total"] + $veh_vehicles_drivers_item["tax"]) . '</td>';
                        break;
