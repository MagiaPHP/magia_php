...
function logs_add_v2($logData) {
    global $db;

    // Verificar que se proporcionaron todos los datos necesarios
    $requiredKeys = ['level',   'u_id', 'u_rol', 'c', 'a', 'w', 'description', 'doc_id', 'crud', 'error', 'val_get', 'val_post', 
        'val_request'];
    ...
    try {
        $req->execute([
            ...
            'doc_id' => $logData['doc_id'],
            'crud' => $logData['crud'],
            'error' => $logData['error'],
            'val_get' => $logData['val_get'],
            'val_post' => $logData['val_post'],
            'val_request' => $logData['val_request'],
        "ip4" => get_user_ip(),
        "ip6" => get_user_ip6(),
        "broswer" => json_encode(get_user_browser())
        ]);
        ...
        // Manejo de errores
        error_log("Error al registrar el log: " . $e->getMessage());
        return false;
    }
}
