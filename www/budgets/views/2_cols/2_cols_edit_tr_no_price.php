<tr style="cursor: all-scroll" class="<?php echo $class; ?>" id="<?php echo "$budget_items[id]"; ?>">  
    ...
    <?php if (!$separator && !$note) { ?> 
        ...
        <?php if ($cbects['description']) { ?>      <td><?php echo "$budget_items[description]"; ?></td> <?php } ?>
        <?php if ($cbects['quantity']) { ?>         <td class="text-right"><?php echo "$budget_items[quantity]"; ?></td> <?php } ?>
        ...
        <?php if ($cbects['description']) { ?>
            <td colspan='<?php echo ( count($cbects) - 7 ); ?>'>
                <?php echo "$budget_items[description]"; ?>
            </td> 
        <?php } ?>
