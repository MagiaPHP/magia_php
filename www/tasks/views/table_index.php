...
<table class="table table-striped table-condensed table-bordered">
    ...
    <thead>
        <tr class="info">
            ...
            $user_cols_array = order_by_get_user_or_default_columns("tasks");

            //vardump($user_cols_array); 
            // Renderizar encabezados con iconos de ordenación
            order_by_render_table_headers_with_icons($user_cols_array, "tasks", $order_data["col_name"], $order_data["order_way"]);
            ?>
        </tr>
    </thead>
    ...
    <tbody>
        ...
        foreach ($tasks as $tasks_item) {
            ...
            $tasks_line = new Tasks();
            $tasks_line->setTasks($tasks_item["id"]);
                

            // Verificar si tiene comentarios
            $has_comments = (comments_total_by_controller_id("tasks", $tasks_line->getId())) ? true : false;
            ...
            if (method_exists($tasks_line, "getDate")) {
                // Llamar al método getDate() si existe
                $date = $tasks_line->getDate();
            ...
            foreach ($user_cols_array as $col) {
                ...
                if ($col["show"]) {
                    ...
                    switch ($field) {
                                         case 'id':
                        echo '<td><a href="index.php?c=tasks&a=details&id=' . $tasks_item['id'] . '">' . ($tasks_item['id']) . '</a></td>';
                        break;
                     case 'category_id':
                        echo '<td>' . ($tasks_item[$field]) . '</td>';
                        break;
                     case 'contact_id':
                            // Obtener id de oficina, si no existe se asigna null
                            $contact_id = $tasks_line->getContact_id() ?? null;
                     ...
                     case 'doc_id':
                        ...
                        break;
                     case 'date_registre':
                        echo '<td>' . magia_dates_formated($tasks_item[$field]) . '</td>';
                        break;
                     case 'date_update':
                        echo '<td>' . magia_dates_formated($tasks_item[$field]) . '</td>';
                        break;
                     ...
                     case "status":
                            ...
                            $status = $tasks_item["status"];

                            // Obtener el ícono y el color del estado
                            $icon_status = icon(tasks_status_field_code("icon", $status));
                            $color = tasks_status_field_code("color", $status);
                            ...
                            echo '<td class="' . $class_status . '">' . $icon_status . ' ' . tasks_status_field_code("name", $status) . '</td>';
                    ...
                    case "total_plus_vat":
                        echo '<td  class="text-right">' . monedaf($tasks_item["total"] + $tasks_item["tax"]) . '</td>';
                        break;
                                            
                    case "button_details":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=tasks&a=details&id=' . $tasks_item['id'] . '">' . icon("eye-open") . ' ' . _tr('Details') . '</a></td>';
                        break;

                    case "button_pay":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=tasks&a=details_payement&id=' . $tasks_item['id'] . '">' . icon("shopping-cart") . ' ' . _tr('Pay') . '</a></td>';
                        break;

                    case "button_edit":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=tasks&a=edit&id=' . $tasks_item['id'] . '">' . icon("pencil") . ' ' . _tr('Edit') . '</a></td>';
                        break;

                    case "modal_edit":
    //                echo "<td>";
    //                include view("tasks", "modal_edit");
    //                echo "</td>";
                        break;

                    case "button_delete":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=tasks&a=delete&id=' . $tasks_item['id'] . '">' . icon("trash") . ' ' . _tr('Delete') . '</a></td>';
                        break;

                    case "modal_delete":
                        echo "<td>";
                        include view("tasks", "modal_delete");
                        echo "</td>";
                        break;

                    case "button_print":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=tasks&a=export_pdf&id=' . $tasks_item['id'] . '">' . icon("print") . '</a></td>';
                        break;

                    case "button_save":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=tasks&a=export_pdf&way=pdf&&id=' . $tasks_item['id'] . '">' . icon("floppy-save") . '</a></td > ';
                        break;
