...
function users_list_by_office_no_root($office_id) {
    ...
    $req = $db->prepare("
            ...
            JOIN 
                rols as r on u.rol = r.rol
            WHERE 
                u.contact_id IN (SELECT id FROM contacts WHERE owner_id = :office_id )  AND u.rol != 'root'
            ORDER BY 
                r.rango DESC, c.lastname, c.name
              ");
...
function verifi_login_password($login, $password) {
    ...
    if ($user['login'] == $login AND password_verify($password, $user['password'])) {
        ...
        $_SESSION['u_owner_id'] = contacts_field_id('owner_id', $user['contact_id']);
