...
# 
if (! banks_lines_status_field_id("id", $id)) {
    array_push($error, "Id is mandatory");
}
#################################################################################
...
if (!$error) {
    $banks_lines_status = new Banks_lines_status();
    $banks_lines_status->setBanks_lines_status($id);

    include view("banks_lines_status", "details");
