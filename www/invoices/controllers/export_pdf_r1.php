...
// de la factura saco el id de la sede
...
if (!$error) {
    ...
    // Presento la lista de facturas que no estan pagadas segun la empresa 
    $invoices = invoices_receivable_by_client($headoffice_id);

    include view("invoices", "export_pdf_r1");

    // redirection 
    switch ($redi) {
        case 1:
            header("Location: index.php?c=invoices&a=details&id=$id#r1");
            break;
