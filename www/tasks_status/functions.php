...
//function tasks_status_can_be_edit($id){
//    return true; 
//}
 ...
 */
function tasks_status_get_user_or_default_columns() {

    // Obtener columnas seleccionadas por el usuario
    $user_cols = user_options("tasks_status_show_col_from_table");
    ...
    if ($user_cols && is_json($user_cols)) {
        ...
        $columns = db_cols_from_table("tasks_status"); // Obtener columnas de la tabla de la base de datos
        //
...
function tasks_status_list($start = 0, $limit = 999, $order_col = "order_by", $order_way = "desc") {
    global $db;
...
// SEARCH
function tasks_status_search($txt, $start = 0, $limit = 999, $order_col = "order_by", $order_way = "desc") {
    global $db;
...
function tasks_status_select($k, $values_to_show = array(), $selected = "", $disabled = array()) {
    $c = "";
...
// SEARCH
function tasks_status_search_by($field, $txt, $start = 0, $limit = 999, $order_col = "order_by", $order_way = "desc") {
    global $db;
...
function tasks_status_add_filter($col_name, $value, $filtre = NULL) {
...
function tasks_status_index_generate_column_headers($colsToShow) {
    foreach ($colsToShow as $col_to_show) {
        switch ($col_to_show) {
            case 'id':
                echo '<th><a href="index.php?c=tasks_status&a=details&id='.$col_to_show.'">' . $col_to_show . '</a></th>';
                break;
...
function tasks_status_index_generate_column_body_td($tasks_status, $colsToShow) {

    foreach ($colsToShow as $key => $col) {

        switch ($col) {
            case 'id':
                echo '<td><a href="index.php?c=tasks_status&a=details&id=' . $tasks_status[$col] . '">' . $tasks_status[$col] . '</a></td>';
                break;
                


case 'id':
                echo '<td>' . ($tasks_status[$col]) . '</td>';
                break;
case 'code':
                echo '<td>' . ($tasks_status[$col]) . '</td>';
                break;
...
case 'status':
                ...
                break;
            case 'button_details':
                echo '<td><a class="btn btn-sm btn-primary" href="index.php?c=tasks_status&a=details&id=' . $tasks_status['id'] . '">' . icon("eye-open") . ' ' . _tr('Details') . '</a></td>';
                break;

            case 'button_pay':
                echo '<td><a class = "btn btn-sm btn-primary" href = "index.php?c=tasks_status&a=details_payement&id=' . $tasks_status['id'] . '">' . icon("shopping-cart") . ' ' . _tr('Pay') . '</a></td>';
                break;


            case 'button_edit':
                echo '<td><a class="btn btn-sm btn-danger" href="index.php?c=tasks_status&a=edit&id=' . $tasks_status['id'] . '">' . icon("pencil") . ' ' . _tr('Edit') . '</a></td>';
                break;
                

            case 'button_delete':
                echo '<td><a class="btn btn-sm btn-danger" href="index.php?c=tasks_status&a=ok_delete&id=' . $tasks_status['id'] . '">' . icon("pencil") . ' ' . _tr('Edit') . '</a></td>';
                break;
                

            case 'button_print':
                echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=tasks_status&a=export_pdf&id=' . $tasks_status['id'] . '">' . icon("print") . '</a></td>';
                break;

            case 'button_save':
                echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=tasks_status&a=export_pdf&way=pdf&&id=' . $tasks_status['id'] . '">' . icon("floppy-save") . '</a></td > ';
                break;
