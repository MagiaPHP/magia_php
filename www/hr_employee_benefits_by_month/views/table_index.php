...
<table class="table table-striped table-condensed table-bordered">
    ...
    <thead>
        <tr class="info">
            ...
            $user_cols_array = order_by_get_user_or_default_columns("hr_employee_benefits_by_month");

            //vardump($user_cols_array); 
            // Renderizar encabezados con iconos de ordenación
            order_by_render_table_headers_with_icons($user_cols_array, "hr_employee_benefits_by_month", $order_data["col_name"], $order_data["order_way"]);
            ?>
        </tr>
    </thead>
    ...
    <tbody>
        ...
        foreach ($hr_employee_benefits_by_month as $hr_employee_benefits_by_month_item) {
            ...
            $hr_employee_benefits_by_month_line = new Hr_employee_benefits_by_month();
            $hr_employee_benefits_by_month_line->setHr_employee_benefits_by_month($hr_employee_benefits_by_month_item["id"]);
                

            // Verificar si tiene comentarios
            $has_comments = (comments_total_by_controller_id("hr_employee_benefits_by_month", $hr_employee_benefits_by_month_line->getId())) ? true : false;
            ...
            if (method_exists($hr_employee_benefits_by_month_line, "getDate")) {
                // Llamar al método getDate() si existe
                $date = $hr_employee_benefits_by_month_line->getDate();
            ...
            foreach ($user_cols_array as $col) {
                ...
                if ($col["show"]) {
                    ...
                    switch ($field) {
                                         case 'id':
                        echo '<td><a href="index.php?c=hr_employee_benefits_by_month&a=details&id=' . $hr_employee_benefits_by_month_item['id'] . '">' . ($hr_employee_benefits_by_month_item['id']) . '</a></td>';
                        break;
                     case 'employee_id':
                            // Obtener id de oficina, si no existe se asigna null
                            $employee_id = $hr_employee_benefits_by_month_line->getEmployee_id() ?? null;
                            ...
                            break;
                     case 'year':
                        echo '<td>' . ($hr_employee_benefits_by_month_item[$field]) . '</td>';
                        break;
                     case 'month':
                        echo '<td>' . ($hr_employee_benefits_by_month_item[$field]) . '</td>';
                        break;
                     ...
                     case 'quantity':
                        ...
                        break;
                     case 'price':
                        echo '<td  class="text-right">' . moneda($hr_employee_benefits_by_month_item[$field]) . '</td>';
                        break;
                     case 'company_part':
                        echo '<td>' . ($hr_employee_benefits_by_month_item[$field]) . '</td>';
                        break;
                    ...
                    case "total_plus_vat":
                        echo '<td  class="text-right">' . monedaf($hr_employee_benefits_by_month_item["total"] + $hr_employee_benefits_by_month_item["tax"]) . '</td>';
                        break;
