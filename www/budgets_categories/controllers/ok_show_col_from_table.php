...
if (!$error) {
    ...
    if (isset($_POST["selected_columns"])) {
        ...
        if ($selectedColumns && is_array($selectedColumns)) {
            ...
            user_options_push_data($u_id, "budgets_categories_show_col_from_table", json_encode($selectedColumns));
            //
            //
        } else {
...
switch ($redi["redi"]) {
        ...
        case 5: // custom
            // ejemplo index.php?c=budgets_categories&a=ok_edit&redi[redi]=5&redi[c]=contacts&redi[a]=details&redi[params]=id%3D1022
            header("Location: index.php?c=" . $redi['c'] . "&a=" . $redi['a'] . "&" . $redi['params'] . "#5");
            break;
