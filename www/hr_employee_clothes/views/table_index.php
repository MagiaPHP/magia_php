...
<table class="table table-striped table-condensed table-bordered">
    ...
    <thead>
        <tr class="info">
            ...
            $user_cols_array = order_by_get_user_or_default_columns("hr_employee_clothes");

            //vardump($user_cols_array); 
            // Renderizar encabezados con iconos de ordenación
            order_by_render_table_headers_with_icons($user_cols_array, "hr_employee_clothes", $order_data["col_name"], $order_data["order_way"]);
            ?>
        </tr>
    </thead>
    ...
    <tbody>
        ...
        foreach ($hr_employee_clothes as $hr_employee_clothes_item) {
            ...
            $hr_employee_clothes_line = new Hr_employee_clothes();
            $hr_employee_clothes_line->setHr_employee_clothes($hr_employee_clothes_item["id"]);
                

            // Verificar si tiene comentarios
            $has_comments = (comments_total_by_controller_id("hr_employee_clothes", $hr_employee_clothes_line->getId())) ? true : false;
            ...
            if (method_exists($hr_employee_clothes_line, "getDate")) {
                // Llamar al método getDate() si existe
                $date = $hr_employee_clothes_line->getDate();
            ...
            foreach ($user_cols_array as $col) {
                ...
                if ($col["show"]) {
                    ...
                    switch ($field) {
                                         case 'id':
                        echo '<td><a href="index.php?c=hr_employee_clothes&a=details&id=' . $hr_employee_clothes_item['id'] . '">' . ($hr_employee_clothes_item['id']) . '</a></td>';
                        break;
                     case 'employee_sizes_clothes_id':
                        echo '<td>' . ($hr_employee_clothes_item[$field]) . '</td>';
                        break;
                    ...
                    case "total_plus_vat":
                        echo '<td  class="text-right">' . monedaf($hr_employee_clothes_item["total"] + $hr_employee_clothes_item["tax"]) . '</td>';
                        break;
                    ...
                    case "modal_edit":
    //                echo "<td>";
    //                include view("hr_employee_clothes", "modal_edit");
    //                echo "</td>";
                        break;

                    case "button_delete":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=hr_employee_clothes&a=delete&id=' . $hr_employee_clothes_item['id'] . '">' . icon("trash") . ' ' . _tr('Delete') . '</a></td>';
                        break;
