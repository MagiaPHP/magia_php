...
/**
 * [EN] Inserts or updates the permissions for a specific role and controller in the database.
 * 
 * [ES] Inserta o actualiza los permisos para un rol y controlador específico en la base de datos.
 * 
 *
 * [EN] If the permissions for the given role and controller already exist, it updates them with the new permissions.
 * 
 * [ES] Si los permisos para el rol y controlador proporcionados ya existen, los actualiza con los nuevos permisos.
 *
 * 
 * [EN] Otherwise, it creates a new entry with the provided permissions.
 * 
 * [ES] De lo contrario, crea una nueva entrada con los permisos proporcionados.
 *
 * @param string $rol [EN] The name of the role for which the permissions will be managed (e.g., 'admin', 'editor').
 *                    [ES] El nombre del rol para el que se gestionarán los permisos (por ejemplo, 'admin', 'editor').
 * @param string $controller [EN] The name of the controller for which the permissions will be managed (e.g., 'users', 'posts').
 *                           [ES] El nombre del controlador para el que se gestionarán los permisos (por ejemplo, 'users', 'posts').
 * @param string $crud [EN] A string representing the CRUD permissions (e.g., '1111' means permission to create, read, update, and delete).
 *                     [ES] Una cadena que representa los permisos CRUD (por ejemplo, '1111' significa permiso para crear, leer, actualizar y eliminar).
 * 
 * @return void
 */
function permissions_push($rol, $controller, $crud) {
    // [EN] Check if a permission record already exists for the given role and controller
    // [ES] Verificar si ya existe un registro de permisos para el rol y controlador proporcionados
    if (permissions_search_by_rol_controller($rol, $controller)) {
        // [EN] If it exists, update the existing permissions
        // [ES] Si existe, actualiza los permisos existentes
        permissions_update($rol, $controller, $crud);
    } else {
        // [EN] If it does not exist, create a new permission record
        // [ES] Si no existe, crea un nuevo registro de permisos
