...
<table class="table table-striped table-condensed table-bordered">
    ...
    <thead>
        <tr class="info">
            ...
            $user_cols_array = order_by_get_user_or_default_columns("hr_employee_category");

            //vardump($user_cols_array); 
            // Renderizar encabezados con iconos de ordenación
            order_by_render_table_headers_with_icons($user_cols_array, "hr_employee_category", $order_data["col_name"], $order_data["order_way"]);
            ?>
        </tr>
    </thead>
    ...
    <tbody>
        ...
        foreach ($hr_employee_category as $hr_employee_category_item) {
            ...
            $hr_employee_category_line = new Hr_employee_category();
            $hr_employee_category_line->setHr_employee_category($hr_employee_category_item["id"]);
                

            // Verificar si tiene comentarios
            $has_comments = (comments_total_by_controller_id("hr_employee_category", $hr_employee_category_line->getId())) ? true : false;
            ...
            if (method_exists($hr_employee_category_line, "getDate")) {
                // Llamar al método getDate() si existe
                $date = $hr_employee_category_line->getDate();
            ...
            foreach ($user_cols_array as $col) {
                ...
                if ($col["show"]) {
                    ...
                    switch ($field) {
                                         case 'id':
                        echo '<td><a href="index.php?c=hr_employee_category&a=details&id=' . $hr_employee_category_item['id'] . '">' . ($hr_employee_category_item['id']) . '</a></td>';
                        break;
                     case 'employee_id':
                            // Obtener id de oficina, si no existe se asigna null
                            $employee_id = $hr_employee_category_line->getEmployee_id() ?? null;
                            ...
                            break;
                      case "category_code":
                        echo '<td>' . hr_employee_category_categories_field_code("category", $hr_employee_category_item["category_code"]) . '</td>';   
                        break;
                     case 'date_start':
                        echo '<td>' . magia_dates_formated($hr_employee_category_item[$field]) . '</td>';
                        break;
                     case 'date_end':
                        echo '<td>' . magia_dates_formated($hr_employee_category_item[$field]) . '</td>';
                        break;
                     case 'order_by':
                        echo '<td>' . ($hr_employee_category_item[$field]) . '</td>';
                        break;
                     case 'status':
                            echo '<td>' . ($hr_employee_category_item[$field]) . '</td>';
                            break;
                    ...
                    case "total_plus_vat":
                        echo '<td  class="text-right">' . monedaf($hr_employee_category_item["total"] + $hr_employee_category_item["tax"]) . '</td>';
                        break;
                                            
                    case "button_details":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=hr_employee_category&a=details&id=' . $hr_employee_category_item['id'] . '">' . icon("eye-open") . ' ' . _tr('Details') . '</a></td>';
                        break;

                    case "button_pay":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=hr_employee_category&a=details_payement&id=' . $hr_employee_category_item['id'] . '">' . icon("shopping-cart") . ' ' . _tr('Pay') . '</a></td>';
                        break;

                    case "button_edit":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=hr_employee_category&a=edit&id=' . $hr_employee_category_item['id'] . '">' . icon("pencil") . ' ' . _tr('Edit') . '</a></td>';
                        break;

                    case "modal_edit":
    //                echo "<td>";
    //                include view("hr_employee_category", "modal_edit");
    //                echo "</td>";
                        break;

                    case "button_delete":
                        echo '<td><a class = "btn btn-sm btn-default" href = "index.php?c=hr_employee_category&a=delete&id=' . $hr_employee_category_item['id'] . '">' . icon("trash") . ' ' . _tr('Delete') . '</a></td>';
                        break;
