...
# Conexión a la base de datos y obtención de la estructura de la tabla
...
En esta entrada, vamos a detallar la estructura y el propósito de la tabla \`$TABLE_NAME\`, que forma parte de la base de datos \`$DB_NAME\`. Esta tabla es utilizada para almacenar información relevante sobre diferentes páginas web y recursos. A continuación, se presenta una descripción completa de su estructura, columnas y relaciones.
...
La columna \`category\` en la tabla \`$TABLE_NAME\` está relacionada con la tabla \`yellow_pages_categories\` mediante una clave foránea:
