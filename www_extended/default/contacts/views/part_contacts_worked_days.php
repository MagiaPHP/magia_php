...
//                    echo '<th><a href="index.php?c=hr_employee_worked_days&order_col=' . $th . '' . $link_order_way . ' ">' . _tr(ucfirst($th)) . ' ' . $order_icon_show . '</a></th>';
//                }
//                $order_icon_show = false;
//            }
            ?>                                
            <?php hr_employee_worked_days_index_generate_column_headers($cols_to_show_keys); ?>
...
//            }
    ...
    <tbody>
        <tr>
            <?php
            $hr_employee_worked_days = hr_employee_worked_days_search_by('employee_id', $id);
            ...
            foreach ($hr_employee_worked_days as $hr_employee_worked_days_item) {
                $menu = '<div class="dropdown">
                            ...
                            <?php if (permissions_has_permission($u_rol, "hr_employee_worked_days", "read")) { ?>
                              <li><a href="index.php?c=hr_employee_worked_days&a=details&id=' . $hr_employee_worked_days_item["id"] . '">' . _tr("Details") . '</a></li>
                            <?php } ?>
                            
                            <?php if (permissions_has_permission($u_rol, "hr_employee_worked_days", "update")) { ?>
                                <li><a href="index.php?c=hr_employee_worked_days&a=edit&id=' . $hr_employee_worked_days_item["id"] . '">' . _tr("Edit") . '</a></li>
                            <?php } ?>    

                             <?php if (permissions_has_permission($u_rol, "hr_employee_worked_days", "delete")) { ?>
                              <li role="separator" class="divider"></li>                              
                              <li><a href="index.php?c=hr_employee_worked_days&a=delete&id=' . $hr_employee_worked_days_item["id"] . '">' . _tr("Delete") . '</a></li>
                            <?php } ?>    
                            

                            </ul>
                          </div>';
                //     echo "<tr id=\"$hr_employee_worked_days_item[id]\">";
                // $checked_array = json_decode(_options_option("config_hr_employee_worked_days_show_col_from_table"), true);
//                foreach (hr_employee_worked_days_db_col_list_from_table($c) as $col_name) {
//                    if (isset($checked_array[$col_name]) || !isset($checked_array)) {
//                        //echo "<td>$hr_employee_worked_days_item[$col_name]</td>";
//                            
//                        // si una columna se llama xx
//                        if ($col_name == "xxxxxxxxxxxxxx") { // se pone un filtro en este caso la primera mayuscula
//                            echo "<td>" . hr_employee_worked_days_add_filter($col_name, $hr_employee_worked_days_item[$col_name], ucfirst($col_name)) . "</td>";
//                        } else {
//                            echo "<td>" . hr_employee_worked_days_add_filter($col_name, $hr_employee_worked_days_item[$col_name]) . "</td>";
//                        }                                                
//                    }
//                }                echo "<td>$menu</td>";
                echo "</tr>";

                //hr_employee_worked_days_index_generate_column_body_td($hr_employee_worked_days_item, $colsToShow);

                foreach ($cols_to_show_keys as $key => $col) {

                    switch ($col) {
                        case 'id':
                            echo '<td><a href="index.php?c=hr_employee_worked_days&a=details&id=' . $hr_employee_worked_days_item[$col] . '">' . $hr_employee_worked_days_item[$col] . '</a></td>';
                            break;
                        case 'id':
    ...
    <?php // include view("hr_employee_worked_days", "table_index_form_add");  ?>
