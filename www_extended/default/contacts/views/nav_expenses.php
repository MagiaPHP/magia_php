...
<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
    ...
    <?php if (permissions_has_permission($u_rol, "invoisssssssces", "create")) { ?>    
        <button 
            type="button" 
            class="btn btn-primary navbar-btn navbar-right" 
            data-toggle="modal" 
            data-target="#contacts_expenses_add"
            >
        ...
        <div class="modal fade" 
             id="contacts_expenses_add" 
             tabindex="-1" 
             role="dialog" 
             aria-labelledby="contacts_expenses_addLabel">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <?php //include "form_contacts_expenses_add.php"; ?>
                    </div>
                </div>
            </div>
        </div>
    ...
    <form class="navbar-form navbar-right" method="get" action="index.php">
        ...
        <div class="form-group">
            <select class="form-control" name="status">
                <option value="all"><?php _t("All"); ?> (<?php echo expenses_total_by_client_id($id); ?>)</option>

                <?php
                foreach (expenses_status_list() as $key => $status_items) {

                    $selected = ($status_items['code'] == $status) ? " selected " : "";

                    $total = expenses_total_by_client_id_status($id, $status_items['code']);
                    ?>
                    <option 
                        value="<?php echo $status_items['code']; ?>" 
                        <?php echo $selected; ?>>
                            <?php _t($status_items['name']); ?> 
...
<?php /**
  ...
  <a class="navbar-brand" href="index.php?c=contacts&a=expenses&id=<?php echo $id; ?>">
