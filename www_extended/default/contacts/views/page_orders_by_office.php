...
<div class="row">
    ...
    <div class="col-sm-2 col-md-2 col-lg-2">
        ...
        <div class="list-group">
            ...
            <a href="index.php?c=contacts&a=orders_by_office&id=<?php echo $id; ?>&status=70" class="list-group-item <?php
            if ($code == 'all') {
            ...
            <?php
            foreach (orders_status_list() as $orders_status_list) {

                $total = orders_total_by_office_id_status($id, $orders_status_list['code']);

                $contador = ($total) ? '<span class="badge">' . $total . '</span>' : "";

                if (rols_status_search_by_status_code($u_rol, $orders_status_list['code'])) {

                    $class = ( $orders_status_list['code'] == $code ) ? " list-group-item list-group-item-info " : " list-group-item ";

                    // No muestro los estatus siguientes
                    // si el status de los pedidos son:
                    // - Anulados (-10)
                    // - Borrador (0)
                    // - finalizado (110)
                    // 
                    if (
                            $orders_status_list['code'] == -10 ||
                            $orders_status_list['code'] == 0 ||
                            $orders_status_list['code'] == 100 ||
                            $orders_status_list['code'] == 110
                    ) {
                        $contador = "";
                    }

                    echo '<a href="index.php?c=contacts&a=orders_by_office&id=' . $id . '&status=' . $orders_status_list['code'] . '" class="' . $class . '">' . _tr($orders_status_list['status']) . '  ' . $contador . '</a>';
                } else {
        ...
        /*
          ...
          <?php
          $j = 1;
          foreach (offices_list_by_headoffice(offices_headoffice_of_office($id)) as $key => $office) {
          echo '<optgroup label="' . contacts_formated($office['id']) . ' ' . $office['id'] . '">';
          //  echo '<option value="'.$office['id'].'">'. contacts_formated($office['id']) .' (headoffice)</option>';
          $i = 1;
          foreach (offices_list_by_headoffice($office['id']) as $key => $office) {
          $selected = ($id == $office['id']) ? " selected " : " ";
          ...
          <button type="submit" class="btn btn-default"><?php echo _t("Change"); ?></button>
          </div>
          ...
          foreach (directory_list_by_contact_id($id) as $directory) {
          ...
          <?php
          foreach (addresses_delivery_list_by_contact_id($id) as $key => $address) {
    ...
    <div class="col-sm-8 col-md-8 col-lg-8">
        <?php include view("contacts", "nav_orders_by_office"); ?>  
        <?php
        if ($orders) {
