<nav class="navbar navbar-default">
    <div class="container-fluid">
        <!-- Brand and toggle get grouped for better mobile display -->
        <div class="navbar-header">
            <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1" aria-expanded="false">
                <span class="sr-only">Toggle navigation</span>
                <span class="icon-bar"></span>
                <span class="icon-bar"></span>
                <span class="icon-bar"></span>
            </button>
            <a class="navbar-brand" href="index.php?c=contacts&a=orders&id=<?php echo $id; ?>">
                <?php _menu_icon("top", "orders"); ?>
                <?php echo ( _options_option('config_menus_debug') ) ? _menus_get_file_name(__FILE__) : _t("Orders"); ?>
            </a>
        </div>

        <!-- Collect the nav links, forms, and other content for toggling -->
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">






            <form class="navbar-form navbar-right" method="get" action="index.php">
                <input type="hidden" name="c" value="contacts">
                <input type="hidden" name="a" value="orders_by_company">
                <input type="hidden" name="id" value="<?php echo $id; ?>">
                <input type="hidden" name="office_id" value="all">




                <div class="form-group">
                    <label for="id" class="sr-only"><?php _t("Company name"); ?></label>
                    <select name="address_id" class="selectpicker" data-live-search="true" data-width="100%">
                        <option value="all"><?php _t('All addresses') ?></option>
                        <?php
                        // Lista de direcciones segun:
                        // empresa 
                        // status 
                        foreach (offices_list_by_headoffice($id) as $key => $office) {



                            // $total = orders_total_by_company_id_status($id, $status_items['code']);
                            ?>
                            <optgroup label="<?php echo ucwords(contacts_formated($office['id'])) ?>">                                                                
                                <?php
                                foreach (addresses_search_by_contact_id($office['id'], "Delivery") as $key => $address) {

                                    $selected = ($address['id'] == $address_id) ? " selected " : "";
                                    ?>


                                    <option value="<?php echo $address['id']; ?>"  <?php echo $selected; ?>   >
                                        <?php echo " [$address[id]] -  $address[number], $address[address] - $address[postcode] $address[city]  "; ?></option>
                                <?php } ?>                               
                            </optgroup>

                            <?php
                            /*

                              <option value="<?php echo $office['id']; ?>" <?php echo $selected; ?>>
                              <?php echo ucwords(contacts_formated($office['id'])) ?>
                              </option>
                             */
                            ?>




                        <?php } ?>
                    </select>
                </div>


                <div class="form-group">
                    <select class="form-control" name="status">
                        <option value="all"><?php _t("All"); ?> <?php //echo orders_total_by_company_id($id);    ?></option>

                        <?php
                        foreach (orders_status_list() as $key => $status_items) {

                            $selected = ($status_items['code'] == $status) ? " selected " : "";

                            $total = orders_total_by_company_id_status($id, $status_items['code']);
                            ?>
                            <option value="<?php echo $status_items['code']; ?>" <?php echo $selected; ?>><?php _t($status_items['status']); ?> <?php //echo ($total) ? "($total)" : "";   ?></option>
                        <?php } ?>
                    </select>
                </div>











                <button type="submit" class="btn btn-default"><?php _t("Search"); ?></button>
            </form>


            <?php
            // crea el menu personalizado 
            _menus_create_menu_items_by_controller_location($c, __FILE__);
            ?>



            <?php
            /*   <button 
              type="button"
              class="btn btn-primary navbar-btn navbar-right"
              data-toggle="modal"
              data-target="#contacts_banks_add"
              >
              <span class="glyphicon glyphicon-plus-sign"></span>
              <?php _t("New") ; ?>
              </button> */
            ?>


        </div><!-- /.navbar-collapse -->
    </div><!-- /.container-fluid -->
</nav>




<div class="modal fade" id="contacts_banks_add" tabindex="-1" role="dialog" aria-labelledby="contacts_banks_addLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="contacts_banks_addLabel">
                    <?php _t("Add new order"); ?>    
                </h4>
            </div>
            <div class="modal-body">
                <p>No es posible agregar una orden desde aca</p>
                <?php //include "form_contacts_banks_add.php" ;   ?>
            </div>


        </div>
    </div>
</div>

