...
//    $permissions = permissions_search_by_rol(users_field_contact_id("rol", $id));
    ################################################################################
    $pagination = new Pagination($page, hr_employee_nationality_search_by("employee_id", $id));
    // puede hacer falta
    $url = "index.php?c=hr_employee_nationality&a=search&w=by_employee_id&employee_id=" . $id;
    $pagination->setUrl($url);
    $hr_employee_nationality = hr_employee_nationality_search_by("employee_id", $id, $pagination->getStart(), $pagination->getLimit());
    ################################################################################ 
////    $permissions = permissions_search_by_rol(users_field_contact_id("rol", $id));
//    ################################################################################
...
//    ################################################################################ 
   ...
   include view("contacts", "page_hr_employee_nationality");
