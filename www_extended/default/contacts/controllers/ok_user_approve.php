...
if (rols_rango_by_rol($u_rol) < rols_rango_by_rol(users_field_contact_id("rol", $contact_id))) {
    array_push($error, "You cannot unblock a user with a higher rank");
}
################################################################################
...
if (!$error) {
    ...
    $w = null;
    // $txt
    $description = "Approuve user: $contact_id";
    $doc_id = $contact_id;
    $crud = "update";
    //$error = null ;
    ...
    switch ($redi) {
        case 1:
            header("Location: index.php?c=contacts&a=system&id=$contact_id&smst=success&smsm=Approved contact");
            break;
        case 2:
            header("Location: index.php?c=contacts&a=users&id=$id&smst=success&smsm=Approved contact");
            ;
            break;
