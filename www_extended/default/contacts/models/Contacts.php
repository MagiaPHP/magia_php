...
class Contact extends Contacts {
...
//    }
    ...
    function getInitials() {
        return strtoupper($this->_initials);
    }
...
//        $this->setInvoices();
    ...
    function setBanks($contact_id) {
        ...
        if (modules_field_module('status', 'banks')) {
            ...
            $banks_list_by_contact_id = banks_list_by_contact_id($contact_id);

            if ($banks_list_by_contact_id) {

                foreach ($banks_list_by_contact_id as $key => $value) {
                    $banco = new Banks();
    ...
    function setInitials() {
        ...
        foreach ($words as $word) {
            if (!empty($word)) { // Check that the word is not empty
                $firstLetters .= strtoupper($word[0]); // Convert to uppercase and add to result
            }
        }

        $this->_initials = $firstLetters;
    }
    ...
    function nameFormated() {
        ...
        $initials = (IS_MULTI_VAT) ? "(" . $this->setInitials() . ") " : "";

        $format = 2;

        switch ($format) {
            case 1:
                $cf = $initials . "" . $this->_name . " " . $this->_lastname;  // robinson coello
                break;
            case 2:
                $cf = $initials . "" . $this->_name . " " . strtoupper($this->_lastname);  // robinson COELLO
                break;
            case 3:
                $cf = $initials . "" . $this->_name . " " . ucfirst($this->_lastname);  // robinson Coello
                break;
            default:
                break;
        }
