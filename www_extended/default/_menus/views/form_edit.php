<form class="form-horizontal" action="index.php" method="post" >
    ...
    <?php if ($arg['izq'] == 'by_location') { ?>
        <input type="hidden" name="redi[params]" value="<?php echo urldecode("w=by_father_id&id=" . $menu->getId() . "&father_id=" . $menu->getId() . "&location=" . $menu->getLocation()) ?>">

    <?php } else { ?>
        <input type="hidden" name="redi[params]" value="<?php echo urldecode("w=by_father_id&id=" . $menu->getId() . "&father_id=" . $menu->getFather_id() . "&location=" . $menu->getLocation()) ?>">
...
//    vardump(icons_list());
    ...
    <?php # icon  ?>
    <div class="form-group">
        <label class="control-label col-sm-3" for="target"><?php _t("Icon"); ?></label>
        <div class="col-sm-8">
            <input 
                type="text" 
                name="icon" 
