...
function modules_list_by_status($status, $start = 0, $limit = 999) {
    global $db;
    $data = null;
    ...
    ORDER BY `module`, `order_by` DESC, `id` DESC  Limit  :limit OFFSET :start  ";
...
function modules_list_without_father_by_status($status, $start = 0, $limit = 999) {
    ...
    ORDER BY `module`, `order_by` DESC, `id` DESC  
...
function modules_update($id, $label, $father, $description, $author, $author_email, $url, $version, $order_by, $status) {

    global $db;
    $req = $db->prepare(" UPDATE `modules` SET `label` =:label, `father` =:father, `description` =:description, `author` =:author, `author_email` =:author_email, `url` =:url, `version` =:version, `order_by` =:order_by, `status` =:status  WHERE `id`=:id ");
    $req->execute(array(
        "id" => $id,
        "label" => $label,
        "father" => $father,
