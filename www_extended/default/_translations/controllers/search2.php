...
if ($w) {
    switch ($w) {
        ...
        case "all":
            ...
            ################################################################################
            if ($language == 'all') {
                $pagination = new Pagination($page, _translations_search($txt));
                // puede hacer falta
                $pagination->setUrl('index.php?c=_translations&a=search&w=all&txt=' . $txt . '&language=all');
                $_translations = _translations_search($txt, $pagination->getStart(), $pagination->getLimit());
            } else {
                // segun idioma
                $pagination = new Pagination($page, _translations_search_by_language($txt, $language));
                // puede hacer falta
                $pagination->setUrl('index.php?c=_translations&a=search&w=byContent&txt=' . $txt . '&language=' . $language . '');
                $_translations = _translations_search_by_language($txt, $language, $pagination->getStart(), $pagination->getLimit());
            }
