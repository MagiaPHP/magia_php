...
if (!file_exists(PATH_GALLERY_IMG_SUBDOMAIN)) {
    echo PATH_GALLERY_IMG_SUBDOMAIN . " no existe";
}
?>
...
<div class="row">
    <?php
    foreach (docu_images_list() as $key => $dil) {

        $img = PATH_GALLERY_IMG_SUBDOMAIN . 'docu/' . $dil["image"];
  ...
  <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3">
    <a href="#" class="thumbnail" data-toggle="modal" data-target="#myModal_' . $dil["id"] . '">
      <img src="' . $img . '" alt="imagen">
    </a>
  </div>  
';

            echo '<div class="modal fade" id="myModal_' . $dil["id"] . '" tabindex="-1" role="dialog" aria-labelledby="myModal_' . $dil["id"] . 'Label">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModal_' . $dil["id"] . 'Label">Modal title</h4>
      </div>
      <div class="modal-body">
        <img src="' . $img . '" alt="imagen">
      </div>
      <div class="modal-footer">
        <a href="index.php?c=gallery&a=ok_delete&id=' . $dil["id"] . 'xx&redi=4" class="btn btn-danger">' . _tr("Delete") . '</a>
