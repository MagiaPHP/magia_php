        ...
        <table class="table table-contents">
            <tbody>
                ...
                foreach (tasks_search_by_controller_doc_id($args['c'], $args['id']) as $key => $task) {
                    ...
                    $icon = icon(tasks_status_field_code('icon', $task["status"]));
  ...
  <div class="modal-dialog" role="document">
    ...
    <ul class="dropdown-menu" aria-labelledby="dropdownMenu_change_status">';
                    ...
                    foreach (tasks_status_list() as $key => $status) {
                        $menu_change_status .= '<li><a href="index.php?c=tasks&a=ok_change_status&id=' . $task["id"] . '&status=' . $status['code'] . '&redi[redi]=3&c=' . $args['c'] . '">' . icon($status ["icon"]) . ' ' . $status ['name'] . '</a></li>';
                    }
