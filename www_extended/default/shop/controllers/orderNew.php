...
$contact_id = (!empty($_GET['contact_id'])) ? clean($_GET['contact_id']) : false;
// 2020-05-08 aaaa-mm-dd
$date_delivery = (!empty($_GET['date_delivery'])) ? clean($_GET['date_delivery']) : false;
// L & R or Stero
$side = (!empty($_GET['side'])) ? clean($_GET['side']) : false;
...
if (!$error) {
    ...
    if ($side == "s") { // stereo
        ...
        include view("shop", "orderNew"); // "www/shop/views/orderNew.php";
    }
    //
} else {
