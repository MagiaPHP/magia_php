...
/**
 * Rol externo, solo puede ver las facturas de su empresa 
 * Verifica si el id que quiere ver pertenece a esta empresa 
 * poner poner el codigo de la factura hashe
 * 
 */
################################################################################
...
if (!invoices_field_code('id', $code)) {
    array_push($error, "Error S-5000");
}
if (!shop_invoices_details_invoice_code($id, $code)) {
...
$invoices = shop_invoices_details_invoice_code($id, $code);
...
if (!$error) {
    ...
    //$a , 
    $w = null;
    $description = "Try print invoice [$id] but error ";
    $doc_id = $id;
    $crud = "read";
    $error = ($error) ? json_encode($error) : null;
    $val_get = ( isset($_GET) ) ? json_encode($_GET) : null;
