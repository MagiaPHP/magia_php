...
if (!$error) {
    switch ($side) {
        case 'L':
            //    $constitution_registred = ($constitution_l_id) ? true : false;
            $has_l_constitution = _types_constitution_list_by_type($type_l_id);
            $has_r_constitution = null;
            break;
        case 'R':
            //    $constitution_registred = ($constitution_r_id) ? true : false;
            $has_l_constitution = null;
            $has_r_constitution = _types_constitution_list_by_type($type_r_id);
            break;
        case 'S':
            //    $constitution_registred = ($constitution_l_id && $constitution_r_id) ? true : false;
            $has_l_constitution = _types_constitution_list_by_type($type_l_id);
            $has_r_constitution = _types_constitution_list_by_type($type_r_id);
            break;
        default:
            //    $constitution_registred = false;
            break;
    }
    ##############################    
    if (($has_l_constitution || $has_r_constitution)) {
        include view("shop", "order_new_9050");
    } else {
        header("Location: index.php?c=shop&a=order_new_9060");
    }
} else {
