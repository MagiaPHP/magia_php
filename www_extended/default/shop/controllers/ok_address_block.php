<?php

/**
 * Bloquea una direccion, para que no sea visible en la lista de entrega
 */
if (!permissions_has_permission($u_rol, "shop_addresses", "update")) {
    header("Location: index.php?c=home&a=no_access");
    die("Error has permission ");
}

################################################################################
# Recolection de variables desde el formulario
################################################################################
$address_id = (!empty($_POST['address_id'])) ? intval(clean($_POST['address_id'])) : false;
$office_id = (!empty($_POST['id'])) ? intval(clean($_POST['id'])) : false;
$redi = (!empty($_POST['redi'])) ? intval(clean($_POST['redi'])) : false;
$error = array();
################################################################################
# Verificacion de varialbes obligatorias
################################################################################
//if (!$address_id) {
//    array_push($error, 'address_id is mandatory');
//}
################################################################################
# Verification de formato de variables obligatorias
# Verdadero control 
################################################################################
//if (users_can_see_others_offices($u_id)) {
//    // esta direccion puede ser de otra oficina 
//    if (offices_headoffice_of_office(addresses_field_id('contact_id', $address_id)) !== contacts_work_for($u_id)) {
//        array_push($error, 'Address does not belong to your company');
//    }
//} else {
//    // debe ser de mi oficina
//    if ((addresses_field_id('contact_id', $address_id)) !== contacts_work_for($u_id)) {
//        array_push($error, 'Address does not belong to your office');
//    }
//}
//
//if (strtolower(addresses_field_id('name', $address_id)) == 'billing') {
//    array_push($error, 'You cannot block a billing address');
//}
################################################################################
# proceso
# if name, lastname an bd is not change, yes you can edit title
################################################################################

$address = new Shop_Addressess();
$address->setAddresses($address_id);
################################################################################
################################################################################
################################################################################
if (!$error && !$address->getErrors()) {

    $address->block();

//------------------------------------------------------------------------------    
// LOGS ------------------------------------------------------------------------
// LOGS ------------------------------------------------------------------------
// LOGS ------------------------------------------------------------------------
    $data_json = json_encode(array(
        $address_id, $office_id
    ));
    ////////////////////////////////////////////////////////////////////////////
    $error = json_encode($error);
    $level = 0; // 1 atention, 2 medio, 3 critico, 4 fatal
    $date = null;
    //$u_id
    //$u_rol , 
    $c = "shop";
    $a = 'adresses_by_office';
    $w = null;
    // $txt
    $description = "Address block [$address_id] ";
    $doc_id = $id;
    $crud = "update";
    //$error = null ;
    //-------------------------------------------------------------------------
    $val_get = (!empty($_GET) ) ? json_encode($_GET) : null;
    $val_post = (!empty($_POST) ) ? json_encode($_POST) : null;
    $val_request = (!empty($_REQUEST) ) ? json_encode($_REQUEST) : null;
    $ip4 = get_user_ip();
    $ip6 = get_user_ip6();
    $broswer = json_encode(get_user_browser()); //https://www.php.net/manual/es/function.get-browser.php
    //-------------------------------------------------------------------------
    logs_add(
            $level, $date, $u_id, $u_rol, $c, $a, $w,
            $description, $doc_id, $crud, $error,
            $val_get, $val_post, $val_request, $ip4, $ip6, $broswer
    );
    ############################################################################
// LOGS ------------------------------------------------------------------------
// LOGS ------------------------------------------------------------------------
// LOGS ------------------------------------------------------------------------
//------------------------------------------------------------------------------

    switch ($redi) {
        case 1:
            header("Location: index.php?c=shop&a=adresses_by_office&id=$office_id");
            break;

        default:
            header("Location: index.php?c=shop");
            break;
    }
} else {


    include view("home", "pageError");
}