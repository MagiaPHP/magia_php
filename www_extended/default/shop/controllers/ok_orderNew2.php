...
if ( ! $address_delivery_id ) {
    array_push($error , '$address_delivery_id not send') ;
}
if ( ! $contact_id ) {
...
if ( ! $error ) {
    ...
    if ( $last_order_id ) {
        ...
        $products_code_l = products_create_code($items_array_left_json) ;
        $products_code_r = products_create_code($items_array_rigth_json) ;
        $products_code_s = products_create_code($items_array_stereo_json) ;

        $products_price_l = products_get_price_by_code($products_code_l) ;
        $products_price_r = products_get_price_by_code($products_code_r) ;
        $products_price_s = products_get_price_by_code($products_code_s) ;

        $products_tax_l = products_get_tax_by_code($products_code_l) ;
        $products_tax_r = products_get_tax_by_code($products_code_r) ;
        $products_tax_s = products_get_tax_by_code($products_code_s) ;
...
// registro los items en las lineas del order
        if ( $items_array_left_json !== null ) {
            shop_orders_add_line($last_order_id , $products_code_l , 1 , $items_array_left_json , $products_price_l , $products_tax_l , "L" , 'bte' , '$model' , '$material' , 0 , '$marca' , '$options' , '$colour') ;
        }

        if ( $items_array_rigth_json !== null ) {
            shop_orders_add_line($last_order_id , $products_code_r , 1 , $items_array_rigth_json , $products_price_r , $products_tax_r , "R" , 'bte' , '$model' , '$material' , 0 , '$marca' , '$options' , '$colour') ;
        }

        if ( $items_array_stereo_json !== null ) {
            shop_orders_add_line($last_order_id , $products_code_s , 1 , $items_array_stereo_json , $products_tax_s , $products_tax_s , "S" , 'bte' , '$model' , '$material' , 0 , '$marca' , '$options' , '$colour') ;
        }
