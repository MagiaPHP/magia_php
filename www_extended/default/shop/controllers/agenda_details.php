...
$txt = (isset($_REQUEST['txt'])) ? clean($_REQUEST['txt']) : false;
$add_place = (isset($_REQUEST['add_place'])) ? clean($_REQUEST['add_place']) : false;
...
if (!$error) {
    ...
    $agenda = shop_agenda_details($id);

    $agenda['category'] = agenda_categories_field_id('category', $agenda['category_id']);
    $agenda['public'] = agenda_public_field_id('public', $agenda['public_id']);

    $agenda['agenda_organizers'] = agenda_organizers_search_by('agenda_id', $id);

    $agenda['places_dates'] = agenda_places_dates_search_by('agenda_id', $id);

    include view("shop", "agenda_details");
} else {
