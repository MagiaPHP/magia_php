<div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title">
            <?php echo contacts_formated(contacts_work_for($u_id)) ?> <?php _t("users"); ?>
        </h3>
    </div>
    <div class="panel-body">
        <?php
        //  vardump(contacts_work_for($u_id));

        $employees = employees_list_by_company(contacts_work_for($u_id));

        //vardump($employees);

        foreach ($employees as $key => $employee) {
            ?>
            <div class="media">
                <div class="media-left">
                    <a href="#">
                        <img width="50" class="media-object" src="data:image/png;base64," alt="...">
                    </a>
                </div>
                <div class="media-body">
                    <h4 class="media-heading"><?php echo contacts_formated($employee['contact_id']) ?></h4>
                    <p><?php _t($employee['cargo']); ?></p>
                </div>
            </div>
        <?php } ?>

    </div>
</div>



<div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title">
            <?php echo $config_company_name; ?>' <?php _t("users"); ?>
        </h3>
    </div>
    <div class="panel-body">
        <?php
        $employees = employees_list_by_company(1022);

        //vardump($employees);

        foreach ($employees as $key => $employee) {
            ?>
            <div class="media">
                <div class="media-left">
                    <a href="#">
                        <img width="50" class="media-object" src="data:image/png;base64," alt="...">
                    </a>
                </div>
                <div class="media-body">
                    <h4 class="media-heading"><?php echo contacts_formated($employee['contact_id']) ?></h4>
                    <p><?php _t($employee['cargo']); ?></p>
                </div>
            </div>
        <?php } ?>

    </div>
</div>