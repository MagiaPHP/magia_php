...
// modelo para la impresion
$m = (isset($_REQUEST["m"]) && $_REQUEST["m"] != "" ) ? clean($_REQUEST["m"]) : 1;
...
# F O R M A T O
#
if (!cv_motivation_letter_is_id($id)) {
    array_push($error, 'Id format error');
}
#################################################################################
# C O N D I C I O N A L
# 
if (!cv_motivation_letter_field_id("id", $id)) {
    array_push($error, "Id is mandatory");
}
#################################################################################
#################################################################################
...
if (!$error) {
    ...
    $cv_motivation_letter = new Cv_motivation_letter();
    $cv_motivation_letter->setCv_motivation_letter($id);

    include view("cv_motivation_letter", "export_pdf");
