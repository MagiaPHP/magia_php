...
<div class="list-group">
    ...
    <a href="index.php?c=banks_lines&a=search&w=by_inout&inout=in" class="list-group-item">
        <?php echo icon("plus"); ?>
        <?php _t("Money in"); ?>
    </a>
    <a href="index.php?c=banks_lines&a=search&w=by_inout&inout=out" class="list-group-item">
        <?php echo icon("minus"); ?>
        <?php _t("Money out"); ?>
    </a>
</div>
...
<div class="list-group">
    ...
    foreach (banks_lines_status_list() as $blst_items) {
        ...
        echo '<a href="index.php?c=banks_lines&a=search&w=by_status&status=' . $blst->getCode() . '" class="list-group-item">' . $total_items . '<span class="' . ($blst->getIcon()) . '"></span> ' . _tr($blst->getName()) . '</a>';
    }
    ?>
    <a href="index.php?c=banks_lines_status" class="list-group-item"><?php echo icon("wrench"); ?> <?php _t('Color configuration'); ?></a>
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <form method="get" action="index.php">
            ...
            <input type="hidden" name="w" value="by_my_account_inout">
            <div class="form-group">
                <label for="account_number"><?php _t("Account number"); ?></label>
                <select name="my_account" class="form-control">
                    <?php
                    foreach (banks_list_by_contact_id_status($u_owner_id, 1) as $key => $blbycisizq) {

                        $selected_bank = (isset($my_account) && $my_account == $blbycisizq['account_number'] ) ? " selected " : "";

                        echo '<option value="' . $blbycisizq['account_number'] . '" ' . $selected_bank . '>' . $blbycisizq['bank'] . ' - ' . $blbycisizq['account_number'] . '</option>';
                    }
                    ?>
                </select>
            </div>
            <div class="form-group">
                <label for="inout"><?php _t("Money in"); ?> / <?php echo _tr("Money out"); ?></label>
                <select name="inout" class="form-control">
                    <option value="all"><?php _t("All"); ?></option>
                    <option value="in"  <?php echo (isset($inout) && $inout == 'in' ) ? " selected " : ""; ?> > <?php _t("Money in"); ?></option>
                    <option value="out" <?php echo (isset($inout) && $inout == 'out') ? " selected " : ""; ?> ><?php _t("Money out"); ?></option>                    
                </select>
            </div>
            <button type="submit" class="btn btn-default">
 ...
 * 
  ...
  <input type="hidden" name="c" value="banks_lines">
  <input type="hidden" name="a" value="search">
  <input type="hidden" name="w" value="by_my_account">
  <div class="form-group">
  <label for="account_number"><?php _t("Account number"); ?></label>
  <select name="my_account" class="form-control">
  <?php
  foreach (banks_list_by_contact_id_status($u_owner_id, 1) as $key => $blbycisizq) {
  echo '<option value="' . $blbycisizq['account_number'] . '" >' . $blbycisizq['bank'] . ' - ' . $blbycisizq['account_number'] . '</option>';
  }
  ?>
  </select>
  </div>
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <form method="get" action="index.php">
            <input type="hidden" name="c" value="banks_lines">
            <input type="hidden" name="a" value="search">
            <input type="hidden" name="w" value="by_my_account_status">
            <div class="form-group">
                <label for="account_number"><?php _t("Account number"); ?></label>
                <select name="my_account" class="form-control">
                    <?php
            ...
            <div class="form-group">
                <label for="account_number"><?php _t("Status"); ?></label>
                <select name="status" class="form-control">
                    <option value="all"><?php _t("All"); ?></option>
                    <?php
                    foreach (banks_lines_status_list() as $key => $blsl) {

                        $selected_status = (isset($status) && $status == $blsl['code'] ) ? " selected " : "";

                        echo '<option value="' . $blsl['code'] . '" ' . $selected_status . '>' . _tr($blsl['name']) . '</option>';
                    }
                    ?>
                </select>
            </div>
...
<?php /**
  ...
  <input type="hidden" name="c" value="banks_lines">
  <input type="hidden" name="a" value="search">
  <input type="hidden" name="w" value="by_my_account_operation">
  ...
  //                        vardump($blbycisizq);
  //
  //                        $selected_my_account = ($blbycisizq['account_number'] = $my_account ) ? ' selected ' : '';
  ...
  $selected_status = ($blsl['code'] = $status ) ? " selected " : "";

  echo '<option value="' . $blsl['code'] . '" '.$selected_status.' >' . $blsl['name'] . '</option>';
  }
  ?>
  </select>
  </div>
