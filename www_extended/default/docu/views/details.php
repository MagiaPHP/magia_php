...
<div class="row">
    <div class="col-sm-12 col-md-3 col-lg-3">
        <?php include view("docu", "izq_details"); ?>
    </div>
    ...
    <div class="col-sm-12 col-md-6 col-lg-6">
        ...
        <h1><?php echo _tr(docu_field_id('title', $id)); ?></h1>
        <?php echo docu_field_id('post', $id); ?>
...
//            echo link_top();
        ...
        foreach (docu_list_of_children($id) as $key => $h2) {

            $icon_edit = (permissions_has_permission($u_rol, 'docu', "update")) ? '<a href="index.php?c=docu&a=edit&id=' . $h2["id"] . '"><span class="glyphicon glyphicon-pencil"></span></a>' : "";

            echo '<h2>' . $icon_edit . '  ' . $h2['id'] . ' <a name="' . $h2['id'] . '"></a>' . $h2['title'] . '</h2>';
//             echo link_to($h2['id']);
            echo '<p>' . $h2['post'] . '</p>';
            echo link_top();
//                echo "<ul>";
            foreach (docu_list_of_children($h2['id']) as $key => $h3) {
                $icon_edit = (permissions_has_permission($u_rol, 'docu', "update")) ? '<a href="index.php?c=docu&a=edit&id=' . $h3["id"] . '"><span class="glyphicon glyphicon-pencil"></span></a>' : "";

                echo '<h3>' . $icon_edit . ' ' . $h3['id'] . ' <a name="' . $h3['id'] . '"></a>' . $h3['title'] . '</h3>';
//                echo link_to($h3['id']);
                echo '<p>' . $h3['post'] . '</p>';
                echo link_top();
...
//                    echo "<ul>";
                foreach (docu_list_of_children($h3['id']) as $key => $h4) {
                    $icon_edit = (permissions_has_permission($u_rol, 'docu', "update")) ? '<a href="index.php?c=docu&a=edit&id=' . $h4["id"] . '"><span class="glyphicon glyphicon-pencil"></span></a>' : "";

                    echo '<h4>' . $icon_edit . ' ' . $h4['id'] . ' <a name="' . $h4['id'] . '"></a>' . $h4['title'] . '</h4>';
//                    echo link_to($h4['id']);
                    echo '<p>' . $h4['post'] . '</p>';
                    echo link_top();
...
//                        echo "<ul>";
                    foreach (docu_list_of_children($h4['id']) as $key => $h5) {
                        ...
                        $icon_edit = (permissions_has_permission($u_rol, 'docu', "update")) ? '<a href="index.php?c=docu&a=edit&id=' . $h5["id"] . '"><span class="glyphicon glyphicon-pencil"></span></a>' : "";

                        echo '<h5>' . $icon_edit . ' ' . $h5['id'] . ' <a name="' . $h5['id'] . '"></a>' . $h5['title'] . '</h5>';
//                        echo link_to($h5['id']);
                        echo '<p>' . $h5['post'] . '</p>';
                        echo link_top();
                        ...
                        foreach (docu_list_of_children($h5['id']) as $key => $h6) {
                            ...
                            $icon_edit = (permissions_has_permission($u_rol, 'docu', "update")) ? '<a href="index.php?c=docu&a=edit&id=' . $h6["id"] . '"><span class="glyphicon glyphicon-pencil"></span></a>' : "";

                            echo '<h6>' . $icon_edit . ' ' . $h6['id'] . ' <a name="' . $h6['id'] . '"></a>' . $h6['title'] . '</h6>';
//                            echo link_to($h6['id']);
                            echo '<p>' . $h6['post'] . '</p>';
                            echo link_top();
                        }
...
//                echo "</ul>";
    ...
    <div class="col-sm-12 col-md-3 col-lg-3">
        <?php include view("docu", "der_details"); ?>
    </div>
